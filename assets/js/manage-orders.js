(function(){
  ...
  function createOrderRow(order, role){
    ...
    return {
      ...
      actionText: 'View',
      actionUrl: resolvePath('/view/order-tracking.html') + '?orderId=' + encodeURIComponent(order.orderId || '') + '&role=' + encodeURIComponent(role)
    };
  }
  ...
  function mapErrandToRow(item, user){
    ...
    var actionUrl = resolvePath('/view/order-tracking.html') + '?kind=errand&id=' + encodeURIComponent(model.id || '') + '&role=' + encodeURIComponent(role);
    if (model.sourceType === 'order' && model.sourceOrderId) {
      actionUrl = resolvePath('/view/order-tracking.html') + '?orderId=' + encodeURIComponent(model.sourceOrderId) + '&kind=order&role=' + encodeURIComponent(role);
    }
